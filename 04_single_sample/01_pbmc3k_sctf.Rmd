...
library(knitr)
opts_chunk$set(echo = TRUE, eval = FALSE, include=TRUE)
```
...
***
*Task: Download the pbmc3k cellranger output file to the current directory that this Rmd file is located and unzip the file.*
<details>
  <summary>where files should be located?</summary>
```{r sample_sheet, fig.align="center", out.width = "60%", fig.cap = 'Fig1. cellranger count output files in your current directory'}
...
By examining the violin plots, we perform QC. We filter cells that have unique feature counts over 2,500 or less than 200. We filter cells that have >10% mitochondrial counts.
```{r after.qc.violin}
...
Each cell has different sequencing depth due to either technical variation or biological variation. Sequencing depth are highly correlated with UMI count but it is also varied depending on each gene. In general, we employ a global-scaling normalization method “LogNormalize” that normalizes the feature expression measurements for each cell by the total expression, multiplies this by a scale factor (10,000 by default), and log-transforms the result.
...
The results of sctransfrom are stored in the 'SCT' assay. The assay has 3 slots: pbmc[['SCT']]@counts, pbmc[['SCT']]@data, and  .scale.data.
