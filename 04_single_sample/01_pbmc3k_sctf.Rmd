...
For this tutorial, we will be analyzing the dataset of Peripheral Blood Mononuclear Cells (PBMC) freely available from 10X Genomics. 2,700 single cells were sequenced on the Illumina NextSeq 500. The raw data can be found [here](https://s3-us-west-2.amazonaws.com/10x.files/samples/cell/pbmc3k/pbmc3k_filtered_gene_bc_matrices.tar.gz).

***
*Task: Download the pbmc3k cellranger output file to the current directory where this Rmd file is located and unzip the file.*
<details>
...
Each cell has a different sequencing depth due to either technical variation or biological variation. Sequencing depth is highly correlated with UMI count but it is also varied depending on each gene. In general, we employ a global-scaling normalization method "LogNormalize" that normalizes the feature expression measurements for each cell by the total expression, multiplies this by a scale factor (10,000 by default), and log-transforms the result.
