...
# immune.combined <- NormalizeData(immune.combined, verbose = FALSE)

theme_set(theme_cowplot())
t.cells <- subset(immune.combined, idents = "CD4 Naive T")
Idents(t.cells) <- "stim"
# avg.t.cells <- log1p(AverageExpression(t.cells, verbose = FALSE)$RNA)
avg.t.cells <- log1p(AverageExpression(t.cells,assays="SCT", slot="data", verbose = FALSE)$SCT)
avg.t.cells$gene <- rownames(avg.t.cells)

cd14.mono <- subset(immune.combined, idents = "CD14 Mono")
Idents(cd14.mono) <- "stim"
# avg.cd14.mono <- log1p(AverageExpression(cd14.mono, verbose = FALSE)$RNA)
avg.cd14.mono <- log1p(AverageExpression(cd14.mono,assays="SCT",slot="data", verbose = FALSE)$SCT)
...
# Here, we use SCT.data that is a log-norm of corrected RNA read count. Each of sample is normalized separately in the SCTransform. If you want more rigorous DGE analysis, you can use RNA.count and ZINB (zero-inflation negative binomial) as a test model.
b.interferon.response <- FindMarkers(immune.combined, assay="SCT", ident.1 = "B_STIM", ident.2 = "B_CTRL", verbose = FALSE)
